...
其中 749 个去重后的学术/医学 URL 已进入 `references/catalog/academic-verification-queue.csv`，当前 674 条已核验、75 条待核验；已核验队列行由 106 条原始研究、32 条综述、21 条观察研究和 515 条仅书目确认组成。队列按被多个 Episode 引用的次数排序；“被引用次数”只表示内容关联度，不表示研究质量或结论强度。研究级记录会把样本、设计、主要结局、争议与外推边界写入备注和证据台账，`verified-bibliographic` 只确认出处存在，不能直接支撑疗效或协议。
