...
维护者本地的 Episode Show Notes 缓存另抽取 851 条指向论文、PubMed、期刊或医学数据库的链接（去重资源节点 6,020 个中的一部分）；原始 `episode-resources.csv` 不随公开仓库分发，公开包只保留去重后的学术核验队列和版权最小化图谱。下一阶段继续建立“播客主张—原始论文—外部复核”三联关系。

其中 749 个去重后的学术/医学 URL 已进入 `references/catalog/academic-verification-queue.csv`，当前 248 条已核验、501 条待核验；已核验队列行由 106 条原始研究、32 条综述、21 条观察研究和 89 条仅书目确认组成。队列按被多个 Episode 引用的次数排序；“被引用次数”只表示内容关联度，不表示研究质量或结论强度。研究级记录会把样本、设计、主要结局、争议与外推边界写入备注和证据台账，`verified-bibliographic` 只确认出处存在，不能直接支撑疗效或协议。

首批核查示例已经显示证据层差异：85% 规则来自计算/模型研究；提取练习来自综述与教育研究；有氧训练—海马研究来自特定老年成人 RCT；短睡眠、急性运动、处方“聪明药”、认知训练长期随访和饮食—睡眠关系各自来自不同的人体研究设计；循环叹息来自短期人体随机研究；视觉威胁与视网膜修复来自动物研究。Skill 输出时必须保留这些证据类型，不能把 Huberman 对论文的科普翻译成统一强度的“人体有效”。
