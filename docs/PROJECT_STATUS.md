...
| Turn evidence into action without knowledge overload | Sixteen outcome-first playbooks, a durable ordinary-language routing corpus and user-guidance contract tests | Implemented for sixteen high-frequency scenarios; 99 first-use routing cases are regression tested and more scenarios remain |
...
The public academic queue contains 1,736 deduplicated Show Notes URLs. Only 684 have bibliographic or stronger verification. Sixty-five high-priority papers, guidelines or tools currently have full structured evidence cards, including five current external alcohol-safety sources that are intentionally outside the Show Notes queue. `verified-bibliographic` confirms identity only; it cannot support efficacy. The remaining queue and future episodes keep the long-term Goal active.
