...
- 按中英文关键词检索研究卡：`python scripts/query_evidence.py "咖啡因 多巴胺"`；自然光/节律证据簇可用 `python scripts/query_evidence.py "自然光 昼夜节律 周末露营"`。输出必须同时保留阴性结果、边界、一手 provenance 和已登记的复制/反证关系；关键词命中只用于定位，不能替代研究解释。
